...
function detectDrowsiness(faceMeshResults){
  ...
  const leftEyeAspectRatio = calculateAspectRatio(landmarks, LEFT_EYE_TOP_BOTTOM, LEFT_EYE_LEFT_RIGHT);
  const rightEyeAspectRatio = calculateAspectRatio(landmarks, RIGHT_EYE_TOP_BOTTOM, RIGHT_EYE_LEFT_RIGHT);
...
function detectGesture(handLandmarkerResults){
  //check next or back
  ...
  if (tan < -5){
    showNotification("Next", "Move detected: Next", 2000);
  }
  else if (tan > 5){
    showNotification("Back", "Move detected: Back", 2000);
  }
  //check stop
  else if(fin1[0].y > fin1[1].y && fin2[0].y > fin2[1].y && fin3[0].y > fin3[1].y && fin4[0].y > fin4[1].y
  && fin1[0].y - fin2[0].y < 0.01 && fin2[0].y - fin3[0].y < 0.01 &&  fin3[0].y - fin4[0].y < 0.01){
    showNotification("Stop", "Move detected: Stop", 2000);
  }
}
...
const CombinedComponent = () => {
  ...
  useEffect(() => {
    const setupFaceMeshAndHandLandmarker = async () => {
      ...
      handLandmarker = await HandLandmarker.HandLandmarker.createFromOptions(vision, {
        baseOptions: {
        ...
        camera = new cam.Camera(webcamRef.current.video, {
          onFrame: async () => {
            ...
            if (webcamRef.current.video) {
              const handResults = await handLandmarker.detect(webcamRef.current.video);
  ...
  const onResults = async ({ faceMeshResults, handResults }) => {
    ...
    if (faceMeshResults) {
      ...
      if (tmp == 0){
        ...
        check = check + 1
        if (check>50){
          showNotification("Drowsy", "Drowsiness Detected", 2000);
          check = 0
        }
      }
    ...
    if (handResults && handResults.landmarks) {
      ...
      detectGesture(handResults.landmarks)
      for (const landmarks of handResults.landmarks) {
        ...
        canvasCtx.beginPath();
        canvasCtx.moveTo(landmarks[0].x * videoWidth, landmarks[0].y * videoHeight);
